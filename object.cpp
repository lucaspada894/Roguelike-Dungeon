...
void gen_consumables(dungeon *d)
{
  object *o;
  std::vector<object_description> &v = d->object_descriptions;
  unsigned int i;
  ...
  for(i = 0; i < v.size(); i++){
    if(v[i].get_name() == "weak Hp Potion"){
      o = new object(v[i]);
      d->vendor_items.push_back(o);
    }
    if(v[i].get_name() == "medium Hp Potion"){
      o = new object(v[i]);
      d->vendor_items.push_back(o);
    }
    if(v[i].get_name() == "greater Hp Potion"){
      o = new object(v[i]);
      d->vendor_items.push_back(o);
    }
    if(v[i].get_name() == "max Hp Potion"){
      o = new object(v[i]);
      d->vendor_items.push_back(o);
    }
    if(v[i].get_name() == "attack potion"){
	o = new object(v[i]);
	d->vendor_items.push_back(o);
      }
    if(v[i].get_name() == "speed potion"){
      o = new object(v[i]);
      d->vendor_items.push_back(o);
    }
    if(v[i].get_name() == "holy dynamite"){
      o = new object(v[i]);
      d->vendor_items.push_back(o);
    }
  }
