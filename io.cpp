...
void io_display_in(dungeon *d)
{
  ...
  mvprintw(6, 10, "Gold: %d", d->PC->gold);
  mvprintw(17, 10, " %-58s ", "");
  mvprintw(18, 10, " %-58s ", "Hit esc to exit, or select a consumable to, uh, consume it.");

  refresh();
  char input;
  while(input = getch()){
    int tempInput = input - '0';
    if (input == 'i' || input == esc){
      io_display(d);
      break;
    }
    else if(tempInput < 0 || tempInput > 9)
      mvprintw(0, 0, "Invalid input");
    else if(d->PC->in[tempInput] == NULL){
	mvprintw(0 ,0, "                                                                                                                             ");
	mvprintw(0, 0, "No item there!");
    }
    else if(d->PC->in[tempInput]->get_type() != 20){
      mvprintw(0, 0, "                                                                                                                                ");
      mvprintw(0, 0, "That's not a consumable!"); 
    }
    else {
      if(strcmp(d->PC->in[tempInput]->get_name(), "weak Hp Potion") == 0){
	if((d->PC->hp + 300) > 1000)
	  d->PC->hp = 1000;
	else
	  d->PC->hp += 300;
      
      d->PC->in[tempInput] = NULL;
      exit = 1;
      break;
      }
      else if(strcmp(d->PC->in[tempInput]->get_name(), "medium Hp Potion") == 0){
	if((d->PC->hp + 500) > 1000)
	  d->PC->hp = 1000;
	else 
	  d->PC->hp += 500;
	...
	break;
      ...
      else if (strcmp(d->PC->in[tempInput]->get_name(), "greater Hp Potion") == 0){
	if((d->PC->hp + 700) > 1000)
	  d->PC->hp = 1000;
	else 
	  d->PC->hp += 700;
	 ...
	 break;
      ...
      else if(strcmp(d->PC->in[tempInput]->get_name(), "max Hp Potion") == 0){
	d->PC->hp = 1000;
      d->PC->in[tempInput] = NULL;
      exit = 1;
      break;
      ...
      else if(strcmp(d->PC->in[tempInput]->get_name(), "attack potion") == 0){
	d->PC->attack_potion_turns = 3;
	d->PC->in[tempInput] = NULL;
	exit = 1;
	break;
      }
      ...
      else if(strcmp(d->PC->in[tempInput]->get_name(), "speed potion")==0){
	d->PC->speed += 10;
	d->PC->speed_on += 1;
	d->PC->in[tempInput] = NULL;
	exit = 1;
	break;
      ...
      else if(strcmp(d->PC->in[tempInput]->get_name(), "flare")==0){
	io_illuminate(d, 1);
	d->PC->in[tempInput] = NULL;
	exit = 1;
	break;
      ...
      else if(strcmp(d->PC->in[tempInput]->get_name(), "holy dynamite")==0){
	int i;
	for(i = 0; i < 10; i++){
	  if((d->PC->in[i]) && strcmp(d->PC->in[i]->get_name(), "flare") == 0){
	    io_grenade_mode(d);
	    move_pc(d, 0);
	    d->PC->in[i] = NULL;
	    d->PC->in[tempInput] = NULL;
	...
	if(i == 10)
	  mvprintw(0, 0, "you need a flare to light up the dynamite and view your surroudings, can only use dynamite if flare is in inventory");
...
void io_handle_input(dungeon *d)
{
  ...
  do {
    ...
    case 'I':
      ...
      case 'B':
	if(isInRange(d))
	io_interact_vendor(d);
      else
	mvprintw(0, 2, "not in range of vendor");
      fail_code = 1;
      break;
    case 'L':
...
{
  /*for(int i = 0; i < DUNGEON_Y + 1; i++){
    for(int j = 0; j < DUNGEON_X; j++){
      mvprintw(i, j, " ");
    }
    }*/

  mvprintw(0, 2, "Would you like to [b]uy or to [s]ell?");
  char input;
  while(input = getch()){
    mvprintw(0, 2, "Would you like to [b]uy or to [s]ell?");
    ...
    if(input == esc){
      ...
      break;
       ...
       io_display_itemsToSell(d);
      mvprintw(0, 0, "                                                                    ");
      mvprintw(0, 0, "Select items to sell");
  

      char sell_input;
      while(sell_input = getch()){
	int tempNum = sell_input - '0';
	io_display_itemsToSell(d);
	if(sell_input == esc){
	  for(int i = 0; i < DUNGEON_Y+1; i++){
	    for(int j = 0; j < DUNGEON_X; j++){
	  ...
	  mvprintw(0, 0, "Would you like to [b]uy or to [s]ell?");
	  break;
	}
	else if(tempNum < 0 || tempNum > 9)
	  mvprintw(22, 2, "Invalid inventory slot");
	else if(d->PC->in[tempNum] == NULL){
	  mvprintw(22, 2, "no item there!");
	}
	else {
	  d->PC->gold += d->PC->in[tempNum]->get_value();
	  // d->vendor_items.push_back(d->PC->in[tempNum]);
	  d->PC->in[tempNum] = NULL;
	  io_display_itemsToSell(d);
	} 
      }
    }
    else if(input == 'b'){
      io_display_vendorItems(d);
      mvprintw(0, 0, "                                                                       ");
      mvprintw(0, 0, "Select items to buy");
      char buy_input;
      while(buy_input = getch()){
	mvprintw(0, 0, "                                                                                                         ");
	mvprintw(0, 0, "Select items to buy");
	int tempNum = buy_input - '0';
	io_display_vendorItems(d);
	...
	else if(tempNum < 0 || tempNum >= static_cast<int>(d->vendor_items.size())){
	  mvprintw(0, 0, "                          ");
	  mvprintw(0, 0, "Invalid inventory slot");
	}
	  else if(d->vendor_items[tempNum] == NULL){
	  mvprintw(0, 0, "                              ");
	  mvprintw(0, 22, "No item there!");
	  }
	else if (d->PC->gold >= d->vendor_items[tempNum]->get_value()){
	    int k;
	    for(k = 0; k < 10; k++){
	      if(d->PC->in[k] == NULL){
		d->PC->in[k] = d->vendor_items[tempNum];
		d->PC->gold -= d->vendor_items[tempNum]->get_value();
        
		d->vendor_items.erase(d->vendor_items.begin() + tempNum);
		k--;
		break;
	      }
	    }
	    if(k == 10){
	      mvprintw(0, 0, "                                       ");
	      mvprintw(0, 0, "No free item slot, please drop an item");
	    }
	    io_display_vendorItems(d);
	}
	else if (d->PC->gold < d->vendor_items[tempNum]->get_value()){
	    mvprintw(0, 0, "                                                 ");
	    mvprintw(0 ,0, "You don't have enough gold for that.");
	  }
      }
    }
...
bool isInRange(dungeon *d)
{
  for(int i = -1; i < 2; i++){
    for(int j = -1; j < 2; j++){
      if(d->character_map[d->PC->position[dim_y]+i][d->PC->position[dim_x]+j]){
	if(strcmp(d->character_map[d->PC->position[dim_y]+i][d->PC->position[dim_x]+j]->name, "Vendor") == 0)
	      return true;
      }
    }
  }
...
void io_display_vendorItems(dungeon* d)
{
  ...
  for(unsigned int i = 0; i < d->vendor_items.size(); i++){
    mvprintw(6, 2, "Gold: %d", d->PC->gold);
    mvprintw(i + 8, 2, "%d: %s", i, d->vendor_items[i]->get_name());
    mvprintw(i + 8, 25, "     cost: %d", d->vendor_items[i]->get_value());
  }
}
...
void io_illuminate(dungeon *d, int delay)
{
  ...
  for(int i = -7; i < 7; i++){
    for(int j = -20; j < 20; j++){
      if(d->PC->position[dim_y] + i > 0 && d->PC->position[dim_y] + i < DUNGEON_Y && d->PC->position[dim_x] + j > 0 && d->PC->position[dim_x] + j < DUNGEON_X){
	
	d->PC->visible[d->PC->position[dim_y] + i][d->PC->position[dim_x] + j] = 1;

	if(d->character_map[d->PC->position[dim_y] + i][d->PC->position[dim_x] + j]){
	  attron(COLOR_PAIR((color = d->character_map[d->PC->position[dim_y] + i][d->PC->position[dim_x] + j]->get_color())));
	  attron(A_BOLD);
	  mvaddch(d->PC->position[dim_y] + 1 + i, d->PC->position[dim_x] + j,
		  character_get_symbol(d->character_map[d->PC->position[dim_y] + i][d->PC->position[dim_x] + j]));
	  attroff(A_BOLD);
	  attroff(COLOR_PAIR(color));
	}
	else if(d->objmap[d->PC->position[dim_y] + i][d->PC->position[dim_x] + j]){
	  attron(COLOR_PAIR(d->objmap[d->PC->position[dim_y] + i][d->PC->position[dim_x] + j]->get_color()));
	  attron(A_BOLD);
	  mvaddch(d->PC->position[dim_y] + 1 + i, d->PC->position[dim_x] + j,
		  d->objmap[d->PC->position[dim_y] + i]
		  [d->PC->position[dim_x] + j]->get_symbol());
	  attroff(A_BOLD);
	  attroff(COLOR_PAIR(d->objmap[d->PC->position[dim_y] + i]
			     [d->PC->position[dim_x] + j]->get_color()));
      }
	else {
	  switch(d->map[d->PC->position[dim_y] + i][d->PC->position[dim_x] +j]){
	  case ter_wall:
	  case ter_wall_immutable:
	  case ter_unknown:
	    attron(A_BOLD);
	    mvaddch(d->PC->position[dim_y] + i + 1, d->PC->position[dim_x] + j, ' ');
	    attroff(A_BOLD);
	    break;
	  case ter_floor:
	  case ter_floor_room:
	...
	case ter_debug:
	  ...
	  case ter_stairs_down:
	    ...
	    break;
	  default:
	    mvaddch(d->PC->position[dim_y] + i + 1, d->PC->position[dim_x] + j, '0');
	  }
	}
      }
    }
  ...
  refresh();
  if(delay == 1){
    mvprintw(0, 0, "use this view to analyze your surroudings, after pressing any key you won't have it any more. Tip: save flare for when you find a dynamite");
    getch();
  }
}
...
void io_grenade_mode(dungeon *d)
{
  ...
  int c;
  io_illuminate(d, 0);
  mvprintw(0, 0, "Select where to drop this most holy of Vatican tools");
  dest[dim_y] = d->PC->position[dim_y];
  dest[dim_x] = d->PC->position[dim_x];
  mvaddch(dest[dim_y] + 1, dest[dim_x], '*');
  ...
  do {
     ...
     for(int i = -2; i < 3; i++){
       for(int j = -4; j < 5; j++){
	 if(dest[dim_y] > 0 && dest[dim_y] < DUNGEON_Y && dest[dim_x] > 0 && dest[dim_x] < DUNGEON_X){
	   if(dest[dim_y] + i < DUNGEON_Y){
	     if(i != 0 || j != 0){
	       attron(A_BOLD);
	       attron(COLOR_PAIR(COLOR_RED));
	       mvaddch(dest[dim_y] + 1 + i, dest[dim_x] + j, '*'); 
	       attroff(COLOR_PAIR(COLOR_RED));
	       attroff(A_BOLD);
	     }
	   }  
     ...
     mvprintw(0, 0, "                                     ");
     mvprintw(0, 0, "%d, %d :: %d, %d", dest[dim_y], dest[dim_x], d->PC->position[dim_y], d->PC->position[dim_x]);
 ...
 switch ((c = getch())) {
    ...
    case KEY_HOME:
      tmp[dim_y]--;
      tmp[dim_x]--;
      if (in_range(tmp[dim_y], tmp[dim_x])) {
        dest[dim_y]--;
	dest[dim_x]--;
      }
      break;
 ...
 mvprintw(0, 0, "                              ");
 mvprintw(0, 0, "%d, %d", dest[dim_y], dest[dim_x]);
 
  } while (c != 'd');/*while (((c == 'g' || c == '.') &&
            (!charpair(dest) || charpair(dest) == d->PC)) ||
	    (c != 'g' && c != '.' && c != 27 )); */
  dynamite_animation(d, dest[dim_y], dest[dim_x]);
  for(int i = -2; i < 3; i++){
    for(int j = -4; j < 5; j ++){
      if(in_range(dest[dim_y] + i, dest[dim_x] + j)){
	if(d->character_map[dest[dim_y] + i][dest[dim_x] + j]){
	    if(3000 > d->character_map[dest[dim_y] + i][dest[dim_x] + j]->hp){
	      io_queue_message("you killed %s ", d->character_map[dest[dim_y] + i][dest[dim_x] + j]->name);
	      d->character_map[dest[dim_y] + i][dest[dim_x] + j]->hp = 0;
	      d->character_map[dest[dim_y] + i][dest[dim_x] + j]->alive = 0;
	      d->character_map[dest[dim_y] + i][dest[dim_x] + j] = NULL;
	      
	    }
	    else {
	      d->character_map[dest[dim_y] + i][dest[dim_x] + j]->hp -= 3000;
	      io_queue_message("you hit %s for 3000 damage!", d->character_map[dest[dim_y] + i][dest[dim_x] + j]->name);
	    }
	}
    }
  }
...
{
  if(row > 0 && row < DUNGEON_Y && col > 0 && col < DUNGEON_X)
    return 1;
  else 
    return 0;
}
...
void display_stats(dungeon *d)
{
  ...
  mvprintw(3, 5, "hp: %d", d->PC->hp);
  mvprintw(5, 5, "speed: %d", d->PC->speed);
  int damage = 0;
  for(int i = 0; i < 12; i++){
    if((d->PC->eq[i])){
  ...
  mvprintw(7, 5, "you're likely to hit for %d", damage);
